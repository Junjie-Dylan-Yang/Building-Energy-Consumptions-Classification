...
building_metadata = read.csv("C:/Users/we704/Desktop/filled_bldg_meta.csv")
...
tmp <- building %>% 
  ...
  select(-X, -site_id, -primary_use, -floor_count, -square_feet, -year_built)
...
building %>% 
  ...
  geom_path()+ggtitle("Cluster 1 Usage Pattern")

building %>% 
  filter(meter == 0, building_id %in% tmpg2) %>%
  mutate(day = wday(timestamp), hour = hour(timestamp)) %>%
  group_by(primary_use, day, hour) %>% 
  summarize(avg_meter_reading = mean(meter_reading)) %>%
  mutate(date = ymd_h(paste0("2020-11-",day, " ", hour))) %>%
  ggplot(aes(x=date, y=avg_meter_reading)) + 
  geom_path()+ggtitle("Cluster 2 Usage Pattern")
...
building %>% 
  ...
  mutate(date = ymd_h(paste0("2020-11-",day, " ", hour))) %>%
  ggplot(aes(x=date, y=avg_meter_reading)) + 
  geom_path()+ggtitle("Cluster 3 Usage Pattern")
